...
describe('parseDeepLink — happy path', () => {
  it('Given electron-l4://action?x=1, returns scheme/action/params', () => {
    const [parsed, err] = parseDeepLink('electron-l4://action?x=1')
  ...
  it('Given electron-l4://log?text=hello+world, returns space-decoded text', () => {
    const [parsed] = parseDeepLink('electron-l4://log?text=hello+world')
    if (!parsed) throw new Error('expected parsed value')
    expect(parsed.action).toBe('log')
    expect(parsed.params.text).toBe('hello world')
  })

  it('Given electron-l4://stop with no params, returns empty params', () => {
    const [parsed] = parseDeepLink('electron-l4://stop')
    if (!parsed) throw new Error('expected parsed value')
  ...
  it('Given multiple params, returns all', () => {
    const [parsed] = parseDeepLink('electron-l4://focus?duration=25&label=deep')
    if (!parsed) throw new Error('expected parsed value')
...
describe('parseDeepLink — boundary cases (R-L4-4)', () => {
  it('Given electron-l4://, returns [null, Error]', () => {
    const [parsed, err] = parseDeepLink('electron-l4://')
  ...
  it('Given a totally malformed URL (electron-l4:/oops), returns [null, Error]', () => {
    const [parsed, err] = parseDeepLink('electron-l4:/oops')
    expect(parsed).toBe(null)
    expect(err).toBeInstanceOf(Error)
  })

  it('Given an unparseable URL ("electron-l4://%"), returns [null, Error]', () => {
    const [parsed, err] = parseDeepLink('electron-l4://%')
    expect(parsed).toBe(null)
    expect(err).toBeInstanceOf(Error)
  })
