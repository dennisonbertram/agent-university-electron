 ...
 *
 * Boot strategy (copied verbatim from L1, then extended):
 ...
 *
 * Extensions for L2:
 *   - `waitForEvents` — wait until N entries with `event` appear.
 *   - `findEvents`    — non-blocking read+filter helper used in assertions.
 *
...
export async function launchApp(extraEnv: Record<string, string> = {}): Promise<LaunchedApp> {
  const logDir = mkdtempSync(path.join(tmpdir(), 'l2-e2e-'))
...
/**
 * Wait until the log file contains at least `count` entries with the given event.
 * Returns the matching entries. Throws on timeout.
 */
