 ...
 * in IPC_REGISTRY must declare a validator + handler. Extended for L5 to
 * enforce that the new updater + crashReporter test seams are present.
 *
 * Carries forward R-L4-1, R-L3-1, R-L2-2.
 */
...
describe('IPC_REGISTRY coverage (L5)', () => {
  ...
  it('Given the registry, L5 updater + crashReporter test seams are present', () => {
    const names = new Set(Object.values(IPC_REGISTRY).map((e) => (e as IpcRegistryEntry).channel))
    expect(names.has(IPC_CHANNELS.TEST_CHECK_FOR_UPDATES)).toBe(true)
    expect(names.has(IPC_CHANNELS.TEST_GET_UPDATER_STATE)).toBe(true)
