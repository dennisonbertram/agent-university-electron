/**
 * IPC argument validators + IpcValidationError class. Extended from L3/L4.
 *
 * New validators at L4 (carried into L5 unchanged):
 *   - traySetState, appGetTrayState, notificationShow,
 *     appSetAutoLaunch, appGetAutoLaunch, appSetTheme, appGetTheme,
 *     dockSetBadge, appAddRecent, testFireShortcut, testEmitPower,
 *     testTriggerWillQuit, testEmitOpenUrl, testEmitSecondInstance.
 *
 * New validators at L5:
 *   - testCheckForUpdates:    no args   (BT-L5-6 / BT-L5-7 driver)
 *   - testGetUpdaterState:    no args   (read-only updater state snapshot)
 *   - testGetCrashReporterState: no args (read-only crashReporter wiring snapshot)
 */
